...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    const checkIfInSection = (x: number, y: number) => {
    ...
    const updateCursorVisibility = (x: number, y: number) => {
      const shouldShowCursor = checkIfInSection(x, y);
      
      if (shouldShowCursor && !isInSection) {
        // Entering section
        isInSection = true;
      ...
      } else if (!shouldShowCursor && isInSection) {
        // Leaving section
        isInSection = false;
      ...
      const leaveHandler = () => {
        ...
        resumeTimeout = setTimeout(() => {
          if (!activeTarget && cursorRef.current && spinTl.current) {
            const currentRotation = Number(gsap.getProperty(
              cursorRef.current,
              "rotation"
            ));
  ...
  }, [targetSelector, spinDuration, moveCursor, constants, hideDefaultCursor, sectionId]);
